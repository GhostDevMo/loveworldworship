...
class NotificationManager {
    ...
    func getNotification(AccessToken: String, completionBlock: @escaping(_ Success: NotificationModel?, _ SessionError: BaseModel.sessionErrorModel?, Error?) ->()) {
        let params = [
        ...
        AF.request(API.Notification_Methods.GET_NOTIFICATION_API, method: .post, parameters: params, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
            if let res = response.value as? [String: Any] {
                ...
                if apiStatus == API.ERROR_CODES.E_TwoH {
                    ...
                    let result = try! JSONDecoder().decode(NotificationModel.self, from: data)
    ...
    func notificationUnseenCount(AccessToken:String,
                                 completionBlock: @escaping (_ Success: notificationUnseenCountModel.notificationUnseenCountSuccessModel?,
                                                             _ SessionError: notificationUnseenCountModel.sessionErrorModel?,
                                                             Error?) ->()) {
        let params = [
            API.Params.AccessToken: AccessToken,
            API.Params.ServerKey: API.SERVER_KEY.Server_Key
        ...
        AF.request(API.Notification_Methods.NOTIFICATION_COUNT_API,
                   method: .post,
                   parameters: params,
    ...
    func updateNotificationSetting(AccessToken:String,
                                   userID:Int,
                                   key:String,
                                   Value:Int,
        ...
        AF.request(API.Notification_Settings.NOTIFICATION_SETTINGS,
                   method: .post,
                   parameters: params,
                   encoding: URLEncoding.default,
                   headers: nil).responseJSON { (response) in
