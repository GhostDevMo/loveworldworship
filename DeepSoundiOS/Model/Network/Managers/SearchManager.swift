...
class SearchManager{
    ...
    func search(AccessToken:String,Keyword:String,GenresString:String,PriceString:String,Limit:Int,Offset:Int,AlbumLimit:Int,AlbumOffset:Int,completionBlock: @escaping (_ Success:SearchModel.SearchSuccessModel?,_ SessionError: String?, Error?) ->()){
        let params = [
            ...
            API.Params.Fetch: "songs,albums,artist,playlist,events,products",
        ...
        AF.request(API.Search_Methods.SEARCH_API, method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
            if (response.value != nil){
                ...
                if apiStatus ==  API.ERROR_CODES.E_TwoH {
                    ...
                    if let error = res["message"] as? String {
                        ...
                        do {
                            ...
                            completionBlock(nil,result.error,nil)
                        }catch(let err) {
                            log.error("error = \(err.localizedDescription)")
                            completionBlock(nil,nil,err)
                        }
