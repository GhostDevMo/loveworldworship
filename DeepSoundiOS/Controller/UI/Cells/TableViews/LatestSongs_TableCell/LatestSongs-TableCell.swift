...
import Async
import DeepSoundSDK
class LatestSongs_TableCell: UITableViewCell {
    ...
    var audioId:String? = ""
    var likedVC:LatestSongsVC?
    var userID:Int? = 0
    var object:DiscoverModel.Song?
     var notLoggedobject:NotDiscoverModel.Song?
    ...
    func notLoggedbind(_ object:NotDiscoverModel.Song, index :Int){
           let thumbnailURL = URL.init(string:object.thumbnail ?? "")
           self.thumbnailImage.sd_setImage(with: thumbnailURL , placeholderImage:R.image.imagePlacholder())
           titleLabel.text = object.title ?? ""
           categoryLabel.text = "\(object.categoryName ?? "") - \(object.publisher?.name ?? "")"
    ...
    func showUserAlert(){
        ...
        let EditSong = UIAlertAction(title:  NSLocalizedString("Edit Song", comment: "Edit Song"), style: .default) { (action) in
            ...
            }else if self.notLoggedobject != nil{
                let vc = R.storyboard.track.uploadTrackVC()
                let object = UpdateTrackModel(trackID: self.notLoggedobject?.id ?? 0, trackName: self.notLoggedobject?.title ?? "", trackTitle: self.notLoggedobject?.title ?? "", trackDescription: self.notLoggedobject?.songDescription ?? "", trackLyrics:  "", tags: self.notLoggedobject?.tags ?? "", genres: self.notLoggedobject?.categoryName ?? "",  price:  0.0, availability: self.notLoggedobject?.availability ?? 0 , ageRestriction: self.notLoggedobject?.ageRestriction ?? 0, downloads:  0,trackImage:self.notLoggedobject?.thumbnail ?? "",songID:self.notLoggedobject?.audioID ?? "")
